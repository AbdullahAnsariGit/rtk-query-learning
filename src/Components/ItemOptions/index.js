    ...
    <ScrollView
      ...
      {itemOptions.map((method, index) => (
        <TouchableOpacity
          ...
          <View style={[styles.iconBg, {backgroundColor: method.selected ? colors.theme.white : colors.background.altGrey}]}>
            <MyIcons name={method.icon} size={17} stroke={method.selected ? colors.background.altGrey : null}/>
          </View>
          <SyneRegular text={method.title} style={{color: method.selected ? colors.theme.white : '#000'}}/>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
